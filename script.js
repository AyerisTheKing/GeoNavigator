// GeoNavigator v6.5 - Main Application Logic
// Features: "Guess Country" Mode (TV Style) + Smart Camera (0.3s transition)
...
class GeoNavigator {
    constructor() {
        ...
        // Координаты центров регионов для умной камеры (Smart Camera)
        this.continentViews = {
            'europe': { center: [50, 15], zoom: 4 },
            'asia': { center: [35, 90], zoom: 3 },
            'africa': { center: [0, 20], zoom: 3 },
            'america': { center: [15, -75], zoom: 3 }, // Центрируем между Северной и Южной
    ...
    init() {
        console.log('GeoNavigator v6.5 initialized');
        this.loadSettings();
    ...
    loadCountryData() {
        if (window.GeoCountries) {
            ...
            console.error('CRITICAL: GeoCountries module not found!');
        }
    }
    ...
    setupEventListeners() {
        ...
        document.getElementById('openAboutBtn')?.addEventListener('click', () => {
            alert(this.getLocalizedText('aboutText'));
        });
    ...
    startGame() {
        ...
        // Включаем CSS-режим для ТВ (скрытие флагов, карта на весь фон)
    ...
    showCapitalByCountryQuestion(q) {
        ...
        flagContainer.innerHTML = `<img src="https://flagcdn.com/w320/${q.code}.png" alt="flag">`;
    ...
    setupCountryInteractivity(feature, layer, map) {
        ...
        // В режиме "Текст" отключаем мышь на карте
        if (this.config.currentGame?.mode === 'countryByCapitalText') {
    ...
    highlightCorrectCountry(name) {
        ...
        this.config.gameState.boundariesLayer.eachLayer(layer => {
            ...
            if (window.GeoCountries?.getCountryNameByCode(iso, adm3) === name) {
                ...
                // В текстовом режиме НЕ летим к стране
                if (this.config.currentGame.mode !== 'countryByCapitalText') {
    ...
    getLocalizedText(key) {
        return LOCALES?.[this.config.settings.language]?.[key] || key;
    }
    
    // Стандартные методы загрузки настроек
